...
+ (instancetype)sharedTSHttpServer {
    ...
    static TSHttpServer *httpServer;
    dispatch_once(&onceToken, ^{
        httpServer = [[TSHttpServer alloc] init];
    });
    return httpServer;
}
...
- (void)_appendLogRecordsToString:(NSMutableString*)string afterAbsoluteTime:(double)time {
    ...
    NSArray<iOSLogMessage *>  *allMsg = [iOSLogManager allLogAfterTime:time];
    [allMsg enumerateObjectsUsingBlock:^(iOSLogMessage * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (NSString *)displayedTextForLogMessage:(iOSLogMessage *)msg{
    NSMutableString *string = [[NSMutableString alloc] init];
    [string appendFormat:@"<td>%@</td> <td>%@</td> <td>%@</td>",[iOSLogMessage logTimeStringFromDate:msg.date ],msg.sender, msg.messageText];
    return string;
